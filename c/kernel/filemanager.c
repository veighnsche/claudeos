...
static void draw_back_arrow(uint32_t *fb, int cx, int cy, uint32_t color) {
  ...
  for (int i = 0; i < arrow_size; i++) {
    ...
    int py_top = cy - i;
    int py_bot = cy + i;
    if (px >= 0 && py_top >= 0 && (uint32_t)px < screen_w &&
        (uint32_t)py_top < screen_h)
      fb[py_top * screen_w + px] = color;
    if (px >= 0 && py_bot >= 0 && (uint32_t)px < screen_w &&
        (uint32_t)py_bot < screen_h)
...
static void fill_rect(uint32_t *fb, int x, int y, int w, int h,
                      uint32_t color) {
...
static void draw_file_icon(uint32_t *fb, int x, int y, int is_folder,
                           uint32_t color) {
  if (is_folder) {
    ...
    for (int i = 0; i < 6; i++) {
      int py = y + 2 + i;
      int px = x + 14 + i;
      if (py >= 0 && px >= 0 && (uint32_t)py < screen_h &&
          (uint32_t)px < screen_w) {
        fb[py * screen_w + px] = color;
      }
    }
  }
...
static void draw_title_bar(uint32_t *fb) {
  ...
  /* Title */
  const char *title =
      editing_file ? "Edit" : (viewing_file ? view_filename : "Files");
  int title_len = 0;
  while (title[title_len])
    title_len++;
  int title_x = (screen_w - title_len * FONT_WIDTH) / 2;
  draw_string(fb, title_x, (TITLE_BAR_HEIGHT - FONT_HEIGHT) / 2, title,
              COLOR_TITLE_TEXT, screen_w, screen_h);
...
static void draw_file_list(uint32_t *fb) {
  ...
  if (file_count == 0) {
    ...
    draw_string(fb, msg_x, screen_h / 2 - FONT_HEIGHT, msg, COLOR_FILE,
                screen_w, screen_h);
    return;
  }
  ...
  for (int i = 0; i < visible_count && (i + scroll_offset) < file_count; i++) {
    ...
    fill_rect(fb, FILE_PADDING, y, screen_w - FILE_PADDING * 2,
              FILE_ROW_HEIGHT - 4, bg_color);
    ...
    /* File name */
    draw_string(fb, FILE_PADDING + 45, y + 8, f->name, COLOR_FILE_TEXT,
                screen_w, screen_h);

    /* File size */
    char size_buf[16];
    format_size(f->size, size_buf);
    draw_string(fb, FILE_PADDING + 45, y + 28, size_buf, COLOR_FILE_SIZE,
                screen_w, screen_h);
  ...
  if (file_count > visible_count) {
    ...
    int thumb_y =
        TITLE_BAR_HEIGHT + 5 +
        (scroll_offset * (total_h - thumb_h)) / (file_count - visible_count);
...
static void draw_file_viewer(uint32_t *fb) {
  ...
  int max_y = editing_file && keyboard_is_visible()
                  ? (int)screen_h - keyboard_get_height() - 40
                  : (int)screen_h - 40;
...
static void draw_status_bar(uint32_t *fb) {
  ...
  if (status_msg[0]) {
    ...
    draw_string(fb, FILE_PADDING, y + 8, "Tap file to view", COLOR_FILE_SIZE,
                screen_w, screen_h);
  }
}
    ...
    if (ev.type == EVENT_TOUCH) {
      ...
      if (ev.subtype == TOUCH_DOWN) {
        if (sy < TITLE_BAR_HEIGHT) {
          if (sx < 50) {
            back_btn_pressed = 1;
          } else if (!viewing_file && sx >= (int)screen_w - 90 &&
                     sx < (int)screen_w - 58) {
            add_btn_pressed = 1;
          } else if (!viewing_file && selected_for_action >= 0 &&
                     sx >= (int)screen_w - 50) {
            del_btn_pressed = 1;
          } else if (viewing_file && !editing_file &&
                     sx >= (int)screen_w - 50) {
            /* Edit button */
            edit_btn_pressed = 1;
        ...
        if (back_btn_pressed && sy < TITLE_BAR_HEIGHT && sx < 50) {
        ...
        } else if (edit_btn_pressed && sy < TITLE_BAR_HEIGHT &&
                   sx >= (int)screen_w - 50) {
          /* Enter edit mode */
          editing_file = 1;
          ...
          keyboard_show();
          status_msg[0] = 0;
        } else if (save_btn_pressed && sy < TITLE_BAR_HEIGHT &&
                   sx >= (int)screen_w - 50) {
          /* Save file */
          save_file();
        } else if (add_btn_pressed && sy < TITLE_BAR_HEIGHT) {
          do_create_file();
        } else if (del_btn_pressed && sy < TITLE_BAR_HEIGHT &&
                   selected_for_action >= 0) {
...
int filemanager_should_close(void) { return want_close; }

void filemanager_clear_close(void) { want_close = 0; }
