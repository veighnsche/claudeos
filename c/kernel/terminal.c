...
static void scroll_up(int lines) {
  int max_scroll =
      history_count > max_visible_lines ? history_count - max_visible_lines : 0;
  scroll_offset += lines;
...
static void cmd_cpu(int argc, char **argv) {
  ...
  if (impl == 0x41)
    ...
    shell_println("Qualcomm");
  else
    shell_println("Unknown");
}
...
static void cmd_color(int argc, char **argv) {
  ...
  if (strcmp(argv[1], "dark") == 0) {
    ...
    color_prompt = 0x0000FF00;
  } else if (strcmp(argv[1], "light") == 0) {
    color_bg = 0x00E0E0E0;
    color_text = 0x00000000;
    color_prompt = 0x00006600;
  } else if (strcmp(argv[1], "matrix") == 0) {
    color_bg = 0x00000000;
    color_text = 0x0000FF00;
    color_prompt = 0x0000AA00;
  } else if (strcmp(argv[1], "ocean") == 0) {
    color_bg = 0x00001133;
    color_text = 0x0066CCFF;
    color_prompt = 0x0000FFFF;
  } else if (strcmp(argv[1], "fire") == 0) {
    color_bg = 0x00220000;
    color_text = 0x00FF6600;
    color_prompt = 0x00FFFF00;
  } else if (strcmp(argv[1], "cyber") == 0) {
    color_bg = 0x00110022;
    color_text = 0x00FF00FF;
    color_prompt = 0x0000FFFF;
  } else {
    /* Custom hex colors */
    color_bg = parse_color(argv[1]);
    if (argc >= 3)
      color_text = parse_color(argv[2]);
    if (argc >= 4)
      color_prompt = parse_color(argv[3]);
  }
  needs_redraw = 1;
...
static void cmd_ws(int argc, char **argv) {
  ...
  if (strcmp(argv[1], "connect") == 0) {
    ...
    case WS_STATE_CLOSED:
      shell_println("Closed");
      break;
    case WS_STATE_CONNECTING:
      shell_println("Connecting");
      break;
    case WS_STATE_OPEN:
      shell_println("Open");
      break;
    case WS_STATE_CLOSING:
      shell_println("Closing");
      break;
    default:
      shell_println("Unknown");
      break;
...
static struct command commands[] = {{"help", cmd_help},
                                    {"close", cmd_close},
                                    {"exit", cmd_close},
                                    {"clear", cmd_clear},
                                    {"heap", cmd_heap},
  ...
  if (http_active) {
    ...
    if (state == HTTP_STATE_DONE) {
      ...
      if (http_req.response.body_len > 0) {
        ...
        if (line_pos > 0)
          shell_flush();
        if (http_req.response.body_len > 500)
          shell_println("...");
      }
  ...
  while (event_pop(&ev) == 0) {
    ...
    if (ev.type == EVENT_KEY) {
      ...
      if (ev.subtype != KEY_PRESS)
        continue; /* Only process key presses */
...
static void draw_title_bar(uint32_t *fb, uint32_t width, int btn_pressed) {
  ...
  draw_string(fb, title_x, (TITLE_BAR_HEIGHT - FONT_HEIGHT) / 2, title,
              TITLE_BAR_TEXT, width, TITLE_BAR_HEIGHT + 50);
...
void terminal_draw(void) {
  ...
  if (scroll_offset > 0) {
    ...
    if (so >= 10)
      scroll_info[pos++] = '0' + (so / 10) % 10;
    scroll_info[pos++] = '0' + so % 10;
    scroll_info[pos++] = ']';
    scroll_info[pos] = 0;
    draw_string(fb, width - 50, content_start_y + 10, scroll_info, 0x0033FF33,
                width, height);
  }
...
int terminal_should_close(void) { return want_close; }

void terminal_clear_close(void) { want_close = 0; }
