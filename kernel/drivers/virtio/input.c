...
/* Current touch state in PIXEL coordinates */
static int32_t touch_x = 360; /* Will be updated to screen center */
static int32_t touch_y = 640;
static int touch_slot = 0;
static int touch_tracking_id = -1;

/* Convert pixel coordinates to normalized 0-32767 range */
static int32_t normalize_x(int32_t px) {
...
static int32_t normalize_y(int32_t py) {
...
static void debug_putc(char c) { *(volatile uint32_t *)UART0_BASE = c; }
...
void virtio_input_set_debug(int enable) { touch_debug = enable; }
...
static void init_virtqueue(struct input_device *dev, int dev_idx) {
  ...
  if (dev->version == 1) {
    ...
    mmio_write(dev->base, VIRTIO_MMIO_QUEUE_DESC_LOW,
               (uint32_t)(uint64_t)dev->desc);
    mmio_write(dev->base, VIRTIO_MMIO_QUEUE_DESC_HIGH,
               (uint32_t)((uint64_t)dev->desc >> 32));
    mmio_write(dev->base, VIRTIO_MMIO_QUEUE_AVAIL_LOW,
               (uint32_t)(uint64_t)dev->avail);
    mmio_write(dev->base, VIRTIO_MMIO_QUEUE_AVAIL_HIGH,
               (uint32_t)((uint64_t)dev->avail >> 32));
    mmio_write(dev->base, VIRTIO_MMIO_QUEUE_USED_LOW,
               (uint32_t)(uint64_t)dev->used);
    mmio_write(dev->base, VIRTIO_MMIO_QUEUE_USED_HIGH,
               (uint32_t)((uint64_t)dev->used >> 32));
...
static void process_input_event(struct input_device *dev,
  ...
  if (ev->type == EV_KEY) {
    if (ev->code == BTN_LEFT || ev->code == BTN_TOUCH ||
      ...
      if (ev->value) {
        /* Button pressed - immediately push TOUCH_DOWN with normalized coords
         */
        touch_is_down = 1;
        touch_tracking_id = 0;
        event_push_touch(touch_slot, TOUCH_DOWN, normalize_x(touch_x),
                         normalize_y(touch_y));
        if (touch_debug) {
        ...
        /* Button released - use normalized coords */
        event_push_touch(touch_slot, TOUCH_UP, normalize_x(touch_x),
                         normalize_y(touch_y));
        touch_is_down = 0;
        touch_tracking_id = -1;
        if (touch_debug)
  ...
  if (ev->type == EV_REL) {
    uint32_t screen_w = goldfish_fb_get_width();
    uint32_t screen_h = goldfish_fb_get_height();
    if (screen_w == 0)
      screen_w = 720;
    if (screen_h == 0)
      screen_h = 1280;
  ...
  if (ev->type == EV_SYN && ev->code == 0) {
    ...
    if (got_touch_data) {
      /* If down, use TOUCH_MOVE. If up, we still push a move event
         so apps can track cursor position even without clicking. */
      event_push_touch(touch_slot, TOUCH_MOVE, normalize_x(touch_x),
                       normalize_y(touch_y));
      if (touch_debug) {
        debug_puts("MOVE: x=");
        debug_hex32(touch_x);
...
void virtio_input_init(void) {
  ...
  for (int i = 0;
       i < VIRTIO_MMIO_COUNT && num_input_devices < MAX_INPUT_DEVICES; i++) {
...
int virtio_input_keyboard_available(void) { return keyboard_available; }

int virtio_input_touch_available(void) { return touch_available; }

int virtio_input_pending(void) { return event_pending(); }
