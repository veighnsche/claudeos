...
/* Background image */
static const image_t background_img = {
    ...
    while (event_pop(&ev) == 0) {
        if (ev.type == EVENT_TOUCH) {
            if (ev.subtype == TOUCH_DOWN) {
                ...
                if (terminal_touch_active && point_in_icon_at(ev.x, ev.y, terminal_icon_x)) {
                    terminal_pressed = 1;
                }
...
static void draw_rect_alpha(uint32_t* fb, int x, int y, int w, int h, uint32_t color, int alpha) {
    for (int py = y; py < y + h && py < (int)screen_h; py++) {
        if (py < 0) continue;
        for (int px = x; px < x + w && px < (int)screen_w; px++) {
            if (px < 0) continue;
...
static void draw_circle_alpha(uint32_t* fb, int cx, int cy, int r, uint32_t color, int alpha) {
    for (int py = cy - r; py <= cy + r; py++) {
        if (py < 0 || py >= (int)screen_h) continue;
        for (int px = cx - r; px <= cx + r; px++) {
            if (px < 0 || px >= (int)screen_w) continue;
...
/* Draw circle border */
static void draw_circle_ring(uint32_t* fb, int cx, int cy, int r, int thickness, uint32_t color) {
    int r_outer = r;
...
static void draw_rounded_rect(uint32_t* fb, int x, int y, int w, int h, int r, uint32_t color) {
    /* Main body */
...
static void draw_rounded_rect_alpha(uint32_t* fb, int x, int y, int w, int h, int r, uint32_t color, int alpha) {
    for (int py = y; py < y + h; py++) {
        ...
        for (int px = x; px < x + w; px++) {
            ...
            if ((px >= x + r && px < x + w - r) || (py >= y + r && py < y + h - r)) {
                ...
                dx = px - (x + r);
                dy = py - (y + r);
                if (dx < 0 && dy < 0 && dx * dx + dy * dy <= r2) inside = 1;
...
static void draw_terminal_icon(uint32_t* fb, int x, int y, int size, int pressed) {
    int cx = x + size / 2;
    int cy = y + size / 2;
    int r = size / 2;
    ...
    draw_string(fb, x + size/2 - 12, y + size/2 - 6, ">_", HOME_TEXT, screen_w, screen_h);
}
            ...
            if (bits & (0x80 >> col)) {
                ...
                for (int dy = 0; dy < 4; dy++) {
                    for (int dx = 0; dx < 4; dx++) {
                        if (px + dx >= 0 && px + dx < (int)screen_w &&
                            py + dy >= 0 && py + dy < (int)screen_h) {
...
static void draw_logo_shiny(uint32_t* fb, int x, int y, const char* str, int phase) {
    int i = 0;
...
void home_draw(void) {
    ...
    int logo_y = (screen_h - logo_h) / 2 - 80;  /* Centered vertically, higher up */
    ...
    draw_rounded_rect_alpha(fb, panel_x, panel_y, panel_w, panel_h, 15, 0x000000, 140);
    ...
    draw_string(fb, tag_x, logo_y + logo_h + 12, tagline, HOME_TEXT_DIM, screen_w, screen_h);
    ...
    if (internet_connected) {
        ...
        draw_string(fb, conn_x, logo_y + logo_h + 32, conn_msg, 0x0000FF88, screen_w, screen_h);
    }
    ...
    /* Draw terminal icon */
    draw_terminal_icon(fb, terminal_icon_x, icon_cy, ICON_SIZE, terminal_touch_active);
    int term_label_x = terminal_icon_x + (ICON_SIZE - 8 * FONT_WIDTH) / 2;
    draw_string(fb, term_label_x, icon_cy + ICON_SIZE + 2, "Terminal", HOME_TEXT_DIM, screen_w, screen_h);
    ...
    draw_string(fb, files_label_x, icon_cy + ICON_SIZE + 2, "Files", HOME_TEXT_DIM, screen_w, screen_h);
...
int home_terminal_pressed(void) {
    return terminal_pressed;
}

int home_files_pressed(void) {
    return files_pressed;
}
